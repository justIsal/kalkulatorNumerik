...
function regulafalsi(x1, x2, xt, e,c){
  ...
  const object = notasii(c,x1,x2,xt,f_x1,f_x2,f_xt,cek);
  array.push(object);
	if (f_xt > 0 && f_xt <= e){
		console.log("Solusi Numerik Ditemukan!\n");
		return xt;
	}else if (Math.abs(f_xt) > 0 && Math.abs(f_xt) <= e){
		console.log("Nilai absolut dari f(" << xt << ") = " << Math.abs(f_xt) << " Lebih kecil dari 0.0001 dan lebih besar dari 0, Maka ini sudah mencapai solusi Numerik yang optimal\n");
		return xt;
	}else if (f_xt == 0){
...
getSubmit.addEventListener('click',(p)=> {
  ...
  console.log(e)
  regulafalsi(x1, x2, x1 - (f(x1) * ((x2-x1)/(f(x2)-f(x1)))), e, 1);
  for(index of array){
